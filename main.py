...
for server in config['servers']:
    ...
    verify_cert=config['servers'][server]['verify_cert']
    )
    qb_client.connect()
    ...
    for category in config['categories']:
        ...
        for torrent in torrents:
            ...
            if ratioMet and ageMet:
                ...
                db.insert_torrent(name=torrent.name,size=torrent.size,ratio=torrent.ratio,added=str(added_on),removed_on=(now),age_when_removed=age_in_hours,seed_time=seedtime.total_seconds(),tracker=torrent.tracker,category=category,server=server)
